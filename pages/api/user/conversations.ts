...
export default async function Conversation(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        console.log("an error has occured while fetching conversations", e)
        res.status(500).json({success: false, message: "an error has occured while fetching conversations"})
    }
}